...
BOOL WINAPI MyGL::removeObject(ULONG_PTR index)
{
	...
	p_entry_byte_indexes = (ULONG_PTR*) HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, ((ULONG_PTR) n_tocopy_entries)*PTR_SIZE_BYTES);
	...
	while(entry_index < n_entries)
	{
		...
		{
			HeapFree(GetProcessHeap(), 0u, p_entry_byte_indexes);
			return FALSE;
		}
...
BOOL WINAPI MyGL::addPolygonObject(const mygl_coord_t *p_coords, UINT32 n_coords, BOOL updateGL)
{
	...
	p_entry = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, entry_size);
	if(p_entry == NULL)
	{
	...
	*((UINT32*) (((ULONG_PTR) p_entry) + __MYGL_ENTRYPOLYGONOBJ_NCOORDS_BYTEOFFSET)) = n_coords;
	...
	HeapFree(GetProcessHeap(), 0u, p_entry);
	p_entry = NULL;
...
BOOL WINAPI MyGL::buffer_alloc(VOID)
{
	...
	this->p_buffer = HeapAlloc(GetProcessHeap(), 0u, this->BUFFER_INIT_SIZE_BYTES);
...
VOID WINAPI MyGL::buffer_free(VOID)
{
	...
	{
		HeapFree(GetProcessHeap(), 0u, this->p_buffer);
		this->p_buffer = NULL;
	}
...
BOOL WINAPI MyGL::buffer_resize(ULONG_PTR new_size_bytes)
{
	...
	p_newbuffer = HeapAlloc(GetProcessHeap(), 0u, new_size_bytes);
	if(p_newbuffer == NULL)
	{
